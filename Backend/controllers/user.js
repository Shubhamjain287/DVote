...
exports.loginUser = async (req,res) => {
    try {
        ...
        if(user.profile){
            return res.status(200).json({message : `User Login Successfully` ,_id: user._id, token, name : user.name , email : user.email, profile: user.profile.url, verified: user.verified, admin: user?.admin});
        }

        return res.status(200).json({message : `User Login Successfully` , _id: user._id, token, name : user.name , email : user.email, verified: user.verified, admin: user?.admin});
